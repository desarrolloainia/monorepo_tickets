...
class GetTicketRequest:
    ...
    async def get(self, request_id: UUID, requester_id: UUID | None = None) -> TicketRequest:
        request = await self.uow.ticket_requests.find_by_id(request_id)
        if request is None or (requester_id is not None and request.created_by_id != requester_id):
